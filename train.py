...
def build_sent(words, poss, w2v_dict, pos_dict, dim, convey, weight):
    '''
...
def build_features(revs, w2v_dict, pos_dict, config):
    ...
    for convey in conveys:
        for i in xrange(len(revs)):
            rev = revs[i]
            sent1 = build_sent(rev["text"][0], rev["pos"][0], w2v_dict, pos_dict, dim, convey, weight)
            sent2 = build_sent(rev["text"][1], rev["pos"][1], w2v_dict, pos_dict, dim, convey, weight)
